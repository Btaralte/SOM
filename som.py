...
for i in range(1,n+1):
    temp_list = list(map(float,input("Enter attributes of input "+str(i)+":").strip().split()))
    input_matrix.append(temp_list)
    if len(temp_list)!=len(input_matrix[0]):
        print("Sorry,the number of attributes of inputs have to be equal!!So I have to terminate this program!!Byee!")
        quit()
    print("")
